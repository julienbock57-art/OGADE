    ...
    <div style={{ background: "var(--bg-panel)", border: "1px solid var(--line)", borderRadius: 12, padding: "16px 20px", marginBottom: 16 }}>
      ...
      {isLoading ? (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 8 }}>
          {filtered.map((p) => (
            <div key={p.id} style={{ position: "relative", aspectRatio: "1", borderRadius: 8, overflow: "hidden", border: "1px solid var(--line)" }}>
              <a href={`/api/v1/fichiers/${p.id}/download`} target="_blank" rel="noreferrer">
                <img
                  src={`/api/v1/fichiers/${p.id}/download`}
